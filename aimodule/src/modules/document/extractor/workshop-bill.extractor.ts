...
export class WorkshopBillExtractor {
  ...
  /** Minimum rows expected for a chunk. Dense CV bills are ~50–65 rows/page; lite stops ~16 without MAX_TOKENS. */
  private minRowsForChunk(
    ...
    // Later terms-only slices (not page 1) — low bar. Whole-doc / first chunk uses 40/page.
    if (isLastChunk && pageIndices[0] !== 1) return Math.max(3, pagesInChunk * 5);
    return pagesInChunk * 40;
  }
  ...
  private isChunkSoftTruncated(
    ...
    if (rowCount === 0) return pageIndices.length > 1;
  ...
  private async extractLeanChunkResilient(
    ...
    let initial: LeanChunkExtractResult | LeanSequentialChunkExtractResult;
    try {
      initial = await this.runSingleLeanChunkExtract(
        inputData,
        pageIndices,
        isFirst,
      ...
      if (!isWorkshopTruncationError(e)) throw e;
      this.obs.warn(
        `WorkshopBillExtractor: Chunk ${chunkIndex + 1} hard-truncated — falling back page-by-page.`,
        { pageIndices },
      );
      initial = tableLayout === 'sequential'
        ? { raw: {}, lineItems: [] }
        : { raw: {}, parts: [], labour: [] };
    }
    ...
    for (let p = 0; p < pageIndices.length; p++) {
      ...
      let sub: LeanChunkExtractResult;
      try {
        sub = await this.runSingleLeanChunkExtract(
          inputData,
          singlePage,
          pageIsFirst,
        ...
        if (!isWorkshopTruncationError(e)) throw e;
        this.obs.warn(
          `WorkshopBillExtractor: Split page ${pageIndices[p]} hard-truncated — skipping remainder of page.`,
        );
        continue;
      }
  ...
  private async extractLeanSequentialChunkResilient(
    ...
    const maxSr = this.maxSerialFromRows(initial.lineItems);
    if (!isTruncated && maxSr > 0 && maxSr > initialRows + 2) {
      this.obs.warn(
        `WorkshopBillExtractor: Sequential chunk ${chunkIndex + 1} Sr.No ahead of rows (maxSr=${maxSr}, rows=${initialRows}). Marking as truncated.`,
      );
      isTruncated = true;
    }
    ...
    for (let p = 0; p < pageIndices.length; p++) {
      ...
      try {
        ...
        if (!isWorkshopTruncationError(e)) throw e;
        this.obs.warn(
          `WorkshopBillExtractor: Sequential page ${pageIndices[p]} hard-truncated — skipping remainder of page.`,
        );
      }
    }
